 ...
 */
public class UpdateMvUtils {
    private static final Logger LOG = LogManager.getLogger(UpdateMvUtils.class);
    ...
    public static Command from(MTMV mv, Set<String> partitionNames,
            Map<TableIf, String> tableWithPartKey, boolean incremental, Map<String, String> incrParams) throws UserException {
        ...
        plan = plan.accept(new PredicateAdder(), new PredicateAddContext(predicates, incrParams));
    ...
    private static Expression convertListPartitionToIn(PartitionItem item, Slot col) {
        List<Expression> inValues = ((ListPartitionItem) item).getItems().stream()
                .map(UpdateMvUtils::convertPartitionKeyToLiteral)
    ...
    public static class PredicateAdder extends DefaultPlanRewriter<PredicateAddContext> {
        ...
        public Plan visitUnboundRelation(UnboundRelation unboundRelation, PredicateAddContext predicates) {
            if (predicates.params != null && predicates.params.size() > 0) {
                unboundRelation = unboundRelation.withScanParams(new TableScanParams(
                        predicates.params.get(TableScanParams.READ_MODE), predicates.params, new ArrayList<>()));
            }
    ...
    public static class PredicateAddContext {
        ...
        public PredicateAddContext(Map<TableIf, Set<Expression>> predicates, Map<String, String> incrParams) {
            this(predicates, null, null, incrParams);
        }

        public PredicateAddContext(Map<BaseTableInfo, Set<String>> partition,
                String partitionName) {
            this(null, partition, partitionName, null);
        }

        public PredicateAddContext(Map<TableIf, Set<Expression>> predicates, Map<BaseTableInfo, Set<String>> partition,
                String partitionName, Map<String, String> incrParams) {
            this.predicates = predicates;
            this.partition = partition;
            this.partitionName = partitionName;
            this.params = incrParams;
        }
